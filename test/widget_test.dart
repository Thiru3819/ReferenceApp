...
void main() {
  test('creates a four digit memory code for user lookup', () async {
    final store = TempleQueueStore();

    final registration = await store.registerVisitor(
      name: 'Meena',
      phone: '9876543211',
      referenceMember: 'Arun',
      ticketCount: 1,
    );

    expect(registration.memoryCode, matches(RegExp(r'^\d{4}$')));
    expect(
      store.findRegistrationByLogin('9876543211', registration.memoryCode),
      same(registration),
    );
  });

  testWidgets('generates a queue number from the registration form', (
    WidgetTester tester,
  ) async {
    ...
    await tester.enterText(
      find.byKey(const ValueKey('registration_name')),
      'Suresh',
    );
    await tester.enterText(
      find.byKey(const ValueKey('registration_phone')),
      '9876543210',
    );
  ...
  testWidgets('logs a temple member in with username and password', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(const TempleQueueApp());
    await tester.pumpAndSettle();
    ...
    await tester.enterText(
      find.byKey(const ValueKey('member_username')),
      'arun',
    );
    await tester.enterText(
      find.byKey(const ValueKey('member_password')),
      'arun@111',
    );
